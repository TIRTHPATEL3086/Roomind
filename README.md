...
<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=4F46E5,8B5CF6,3B82F6,6366F1&height=280&section=header&text=RoomMind&fontSize=110&fontAlignY=38&desc=Intelligent%203D%20Spatial%20AI%20%26%20Embodied%20Robotics&descAlignY=60&descAlign=50&fontColor=ffffff" width="100%" alt="RoomMind Banner" />
...
*The GSAP-powered landing page — 60fps 3D room preview, live stats, and instant demo launch*
...
*ARIA navigating to `chair_01`, real-time joint telemetry, semantic object labels, Quick Commands panel*
...
│   ├── app/
│   │   ├── api/v1/        # REST endpoints: rooms, chat, robot, imagine
...
├── 📡  infra/amqtt/       # MQTT broker config
...
| **Infrastructure** | `Docker`, `Render`, `GitHub Actions` | Multi-stage build, cloud deployment, CI |
...
```powershell
# 1. Clone the repo
git clone https://github.com/TIRTHPATEL3086/Roomind.git
cd Roomind

# 2. Set up environment variables
cp .env.example .env
# Fill in: GROQ_API_KEY, DATABASE_URL, DATABASE_URL_SYNC

# 3. Create backend venv and install (Python 3.11 required)
py -3.11 -m venv backend\.venv
backend\.venv\Scripts\python -m pip install -r backend\requirements-deploy.txt
...
# 5. Seed the demo room
backend\.venv\Scripts\python.exe scripts\seed_demo_room.py
...
Fully deployed on **Render** with a single `render.yaml` Blueprint. Push to `main` to trigger auto-deploy.

```bash
git push origin main   # → Render auto-deploys in ~5 minutes
```

**Infrastructure:**
- 🌐 **Web App** → [Render](https://render.com/) (Docker, single container)
- 🐘 **Database** → [Neon Cloud PostgreSQL](https://neon.tech/)
- 📡 **MQTT Broker** → Embedded `amqtt` inside the container
...
# Chat with ARIA (spatial reasoning)
POST /api/v1/chat
# Body: {"message": "Where is the chair?"}

# Robot controls
POST /api/v1/commands          # navigate, wave, dance, look_at...
POST /api/v1/estop             # 2ms emergency stop

# Generative 3D
POST /api/v1/imagine           # Upload image → 3D object placed in room

# Real-time (WebSocket)
WS   /ws                       # Robot pose, telemetry, scene events
```
